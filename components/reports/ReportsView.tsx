...
const ReportCard: React.FC<{title: string; description: string;}> = ({ title, description }) => (
    <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-lg p-6 flex flex-col justify-between">
        <div>
            <h3 className="text-lg font-bold text-white mb-2">{title}</h3>
            <p className="text-sm text-gray-400 mb-4">{description}</p>
        </div>
        <button className="w-full mt-2 bg-cyan-500/20 hover:bg-cyan-500/40 text-cyan-300 font-semibold py-2 px-4 rounded-md text-sm transition-colors flex items-center justify-center">
  ...
  return (
    <div className="animate-fadeIn">
       <div className="text-left mb-8">
        <h2 className="text-3xl font-extrabold text-white mb-2">Reports</h2>
        <p className="text-gray-400">Generate and download key business reports.</p>
      </div>
